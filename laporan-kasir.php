              ...
              <div class="table-auto">
                <table id="laporan-transaksi-kasir" class="table table-bordered table-striped table-laporan">
                  ...
                  <?php 
                    $i = 1; 
                    $total = 0;
                    $queryInvoice = $conn->query("SELECT invoice.invoice_id ,invoice.penjualan_invoice, invoice.invoice_tgl, user.user_id, user.user_nama, invoice.invoice_total, invoice.invoice_sub_total
                               FROM invoice 
                               JOIN user ON invoice.invoice_kasir = user.user_id
                               WHERE invoice_date BETWEEN '".$tanggal_awal."' AND '".$tanggal_akhir."' && invoice_cabang = $sessionCabang && invoice_draft = 0
